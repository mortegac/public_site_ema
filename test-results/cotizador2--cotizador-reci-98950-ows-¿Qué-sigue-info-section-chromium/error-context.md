...
- Name: cotizador2.spec.ts >> /cotizador/recibo-pago — with valid paymentData >> shows "¿Qué sigue?" info section
- Location: e2e/cotizador2.spec.ts:783:7
...
Locator: getByText('¿Qué sigue?').first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for getByText('¿Qué sigue?').first()
